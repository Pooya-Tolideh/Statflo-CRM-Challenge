...
//Contstuctor will set the initial state to a Account Object that is null
//It also sets Modal page state to false
//componentWillMount() launches the page by calling the handler and setting open = true

//componnentDidMount() will updates the state and Account Object with the chosen Account that was clicked on
...
//Interacting with input fields will update the state and the value of the the Account Obj

//if Cancel Button,then window goes back to previous state
//if Save Button, then state value/new Account info is placed inside an array called 'update'
//Then update value is dispatched using updateList action
//New Account is then updated inside dataBase reducer
...
import { updateList , getSnack, setPage , setListFilter } from '../actions/index.js'
    ...
    render() {
        const actions = [
            <SaveButton onClick = {e => {
                        ...
                        this.props.updateList(update,'ACCOUNT')
